...
- [completed] 16. 生成记录独立 Tab
  ...
  - 补组件契约测试并运行相关回归。
- [in_progress] 17. SAM 服务内置与可配置启动时机
  - 将外部 `sam_server` 的可提交文件迁入模板插件 `sam/sam_server/`，不提交真实 `.env`。
  - 为插件 `info.json` 增加外部脚本启动配置，支持 `server-load` 与 `first-request`。
...
| 可选插件测试 `rg` 无匹配返回 1，导致并行读取整体失败 | 1 | 将必读文件和可选搜索拆开执行，并允许无匹配 |
| 迁移前路径保护因 `/` 与 `\\` 格式不同误判目标越界 | 1 | 先创建并 Resolve-Path 目标，再以同格式绝对路径校验父目录后移动 |
| SAM 全量插件测试中的既有网络错误上下文用例失败 | 1 | 确认模板/运行副本一致且属于阶段 6 未完成项；新增启动配置测试移到独立文件，本阶段不扩大修改范围 |
