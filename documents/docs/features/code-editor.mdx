...
前提：workspace 必须有有效的绑定目录，后端进程能访问该目录，项目依赖与类型声明需实际存在于文件系统中。详见 [Monaco TypeScript LSP](/docs/monaco-typescript-lsp)。
...
基于 dom-inspector-hook 的元素源码定位功能，详见 [DOM Inspector 集成](/docs/dom-inspector-integration)：
