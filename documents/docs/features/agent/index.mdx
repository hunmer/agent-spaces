...
| Pi | @earendil-works/pi-coding-agent | 多模型支持 | 原生 SDK 会话、工具、技能、MCP 与流式事件 |

运行时通过工厂函数 `createAgentRuntime()` 按 `runtimeKind` 切换（可选值：`claude-code` / `codex` / `open-agent-sdk` / `langchain` / `hermes` / `pi`），无需修改代码即可更换 Agent 的底层运行时。各运行时在底层共享同一套 Function Call 工具抽象（见下文），但模型加载、session 管理、工具暴露方式由各适配器自行实现。
...
在 Claude Code 运行时中，工具通过 SDK MCP Server 暴露，模型实际调用名为 `mcp__agent-spaces__<工具名>`（例如 `mcp__agent-spaces__WriteDatabaseNode`）。Codex 运行时使用仅监听 `127.0.0.1` 的短生命周期本地 Streamable HTTP MCP server；Pi 运行时则直接注册原生 custom tools。
