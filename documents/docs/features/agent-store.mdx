...
Agent Store 是 Agent Spaces 的**在线模板市场**。它把仓库内置的 `packages/templates` 模板库以静态资源方式对外暴露，让用户无需手动复制文件，就能在侧边栏各类「商店」对话框里浏览、搜索并一键导入 Agent 预设、Chat 助手、MCP 服务器、Skill、Plugin、Workflow、Prompt、输出风格、Mini-app、技能包（Skill Package）等十类资源。
...
`packages/templates` 下按子目录组织十类模板，每类目录都有对应的 `index.json` 索引：
...
| 技能包 | `skillspackage/` | 目录（`manifest.json` + `PROMPT.md` + `skills/*.zip`） | 约 1 个 |
...
3. **计算 MD5 指纹**——文件级用 `fileMD5`，目录级（Skill / Plugin / Mini-app / 技能包）用 `folderMD5`（递归哈希所有文件后聚合）
4. **保留 `updatedAt`**：若 MD5 与上次索引一致则沿用旧时间戳，否则取最新 mtime，避免索引无谓变动
5. **写出 `index.json`**；技能包额外会把整个目录打成 `{slug}.zip`（store + deflate 混合，zero-dep 手写打包器），MD5 变化时才重打包
...
| `scanSkillsPackageStore` | `generate-index.mjs:461` | `{ id, name, summary, skillSlugs, skillCount, zipUrl, md5, updatedAt }` |
...
| `sidebar/skills-package-dialog.tsx` | `skillspackage/index.json` | 技能包 |
...
技能包是「Agent 预设 + 若干私有 Skill」的组合包。安装时会自动创建一个 agent 模板，把 `PROMPT.md` 转成 `systemPrompt`，并解压包内每个 `skills/*.zip` 到该 agent 的私有 skills 目录。

- SDK：`packages/sdk/src/modules/skills-package.ts` → `POST /api/skills-packages/install`，body `{ zipUrl }`
- 后端服务：`packages/server/src/services/skills-package.ts` 的 `installSkillsPackage(zipBuffer)`
- 后端路由：`packages/server/src/routes/skills-package.ts`

流程参见 `skills-package-dialog.tsx` 的 `handleInstall`：前端解析出商店 zip 完整 URL 传给后端，后端 `fetch` 下载 zip 后解压、读 `manifest.json`/`PROMPT.md`，按 `templateId = manifest.slug` 去重（已存在则复用 agentId 更新，否则新建 uuid），解压 skills zips 到 `agent-templates/{agentId}/skills/{skillSlug}/`，最后写 `agent.json`（`skills` = manifest 声明 ∩ 实际解压成功，`tools` 默认空，`modelProvider`/`modelId`/`icon`/`avatarUrl` 可在 manifest 中覆盖）。

卸载走 `DELETE /api/skills-packages/:slug`（`sdk.skillsPackage.uninstall(slug)`），删除对应的 agent 模板及其私有 skills 目录。

> 技能包创建的 agent 默认不开启任何内置工具（`tools: []`），保持最小权限；如需开启，在 manifest 中显式声明 `tools` 数组（值为合法的 `BuiltInAgentToolName`，非法名会被过滤）。
...
- **技能包**：在 `skillspackage/{slug}/` 下放置 `manifest.json`（含 `slug`/`displayName`/`summary`/`skillSlugs`，可选 `modelProvider`/`modelId`/`icon`/`avatarUrl`/`tools`）、`PROMPT.md`（系统提示词）和 `skills/*.zip`（每个 zip 内 `SKILL.md` 在根目录）
