...
- 2026-07-30：开始阶段 18，定位 Spine attachment 命中、骨骼树滚动和短暂高亮的数据流。
...
- 2026-07-30：开始阶段 17；确认现有插件加载器没有外部脚本协议，决定新增 `startup` 配置并在 server-load/首次 tools-workflow 执行两个时机接入，真实 `.env` 不迁入仓库。
- 2026-07-30：SAM Python 服务四个可提交文件已迁入模板插件 `sam_server/`，源目录仅保留真实 `.env`；运行时 sidecar 管理、两种触发入口、就绪等待和聚焦测试已落地。
