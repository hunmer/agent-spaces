...
Workflow UI 渲染器 (`mini-app-renderer.tsx`) 在浏览器端编译和运行用户编写的 React/HTML 代码，无需服务端构建。
...
- 宿主已有第三方依赖但 workflow 需要直接 import 时，在 `mini-app-renderer.tsx` 的外部模块映射中添加明确 allowlist。
...
- 后端把这次执行登记为 running 任务（进程内 cache，按 projectId 分组，位于 `mini-app-tasks.ts`），广播 `miniApp.taskStarted`。
...
| 事件 | 触发时机 | payload |
|------|----------|---------|
| `miniApp.taskSnapshot` | 客户端连入/重连时由服务端推送 | `{ tasks: MiniAppTask[] }`（当前所有 running + 最近终态，终态保留 10min） |
| `miniApp.taskStarted` | execute 开始 | `{ taskId, executorId, pluginId, toolName, meta }` |
| `miniApp.taskFinished` | execute 成功 | `{ taskId, executorId, pluginId, toolName, meta, result }` |
| `miniApp.taskFailed` | execute 抛错 | `{ taskId, executorId, pluginId, toolName, meta, error }` |

订阅：

```js
const unsubscribe = window.AgentSpaces.onTaskEvent((event, data) => {
  if (event === 'miniApp.taskSnapshot') { /* 用 data.tasks 重建队列视图 */ }
  else if (event === 'miniApp.taskStarted') { /* 追加/更新 running 项 */ }
  else if (event === 'miniApp.taskFinished') { /* 解析 data.result 落库 */ }
  else if (event === 'miniApp.taskFailed') { /* 标记失败 */ }
});
```
...
- **收敛在宿主层**：任务 cache、事件广播、snapshot 推送都在 server 宿主层完成；mini-app 项目代码只负责「订阅事件 + 传 taskId/meta」，不要在项目内自行实现跨标签同步、轮询状态机或任务队列持久化。
...
| `ctx.writeConfig(path, value)` | 写 `configs/<path>`，随后广播 `miniApp.configChanged { path, value }` |
...
UI 不再直接 `readConfigJson/writeConfigJson`：落库走 `invokeService`，读取走 `getConfig`/`onConfigChanged`。REST 入口：`POST /api/mini-apps/:id/services/invoke`，body `{ name, payload }` → `{ ok, result }`。
...
- `POST /api/mini-apps/:id/db/:dbName`，body `{ sql, params?, mode: 'all'|'get'|'run'|'exec' }` → `{ ok, result }`。
- `POST /api/mini-apps/:id/db/:dbName/transaction`，body `{ statements: [{ sql, params? }] }` → `{ ok }`。
- 错误：`{ ok: false, error: { code, message } }`，HTTP 400（SQL 错误、库名非法、越界、ATTACH 拦截、行数超限等）。
- 文件位置：`~/.agent-spaces-data/mini-apps/<projectId>/data/db/<name>.sqlite`。
...
| `packages/server/src/storage/mini-app-db.ts` | SQLite 层：better-sqlite3 连接池、`executeDb` / `executeDbTransaction` / `closeProjectDbs`、库名校验与 ATTACH 拦截 |
| `packages/server/src/services/mini-app-tasks.ts` | WS 任务状态 cache（`startTask`/`finishTask`/`failTask`/`listTasks`/`prune`） |
| `packages/server/src/services/mini-app-services.ts` | 项目 services 加载器：编译 `src/services/*.js`、按 projectId 缓存、`invokeService`、注入 ctx |
| `packages/server/src/routes/plugin.ts` | execute 路由，任务编排与 `miniApp.*` 广播 |
| `packages/server/src/ws/handler.ts` | `onClientConnected` 推送 `taskSnapshot` |
| `packages/shared/src/types/events.ts` | `MiniAppTask` 类型与 4 个 `miniApp.*` 事件契约 |
