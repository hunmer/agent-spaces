...
- `todo`：统一 chain 容器，包含 AI 中间消息 step 和工具调用 step
- `terminal`：命令和终端输出
...
- `todo` part 是统一 chain 容器，前端用 `chain-of-thought.tsx` 展示。
...
3. `todo` part 渲染统一 chain，包含 AI 中间消息和工具 step
...
| `todo` | `chain-of-thought.tsx` | 统一 chain：AI 中间消息和工具调用步骤 |
