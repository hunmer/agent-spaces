...
- **Agent 运行时**：支持两种运行时 -- `OpenAgentSdkRuntime`（基于 @codeany/open-agent-sdk）和 `ClaudeCodeRuntime`（基于 @anthropic-ai/claude-agent-sdk），通过工厂函数 `createAgentRuntime()` 按配置切换
...
| 模块 | 路径 | 语言 | 职责 |
|------|------|------|------|
| shared | `packages/shared` | TypeScript | 前后端共享类型定义（Workspace, Issue, Task, Agent, Channel, Message, Event, File, Git, LLM） |
| server | `packages/server` | TypeScript | Express REST API + WebSocket 服务 + Agent 编排 + PTY 终端 + Git 操作 + JSON 持久化 + LLM 模型/供应商管理 + Agent Preset 管理 |
| web | `packages/web` | TypeScript/TSX | Next.js 前端 SPA，包含代码编辑器、终端、聊天（TipTap 富文本 + mention）、议题管理、Git 面板、Agent 配置、LLM 管理 |
...
2. 配置 Agent Preset（角色、运行时类型、模型、API Key、MCP、技能等）
3. 创建议题（Issue）-> 调度者自动唤醒策划者 -> 策划者分解任务
4. 执行者执行任务 -> 审核者审核 -> 议题状态流转
5. 也可在频道聊天中 @mention Agent 直接触发执行
6. 所有状态变更通过 WebSocket 实时推送到前端
...
- **Agent 运行时测试**：OpenAgentSdkRuntime / ClaudeCodeRuntime 的 execute/stop 行为
