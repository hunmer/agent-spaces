...
/**
 * UI 拆分节点：接收上游单图或本地上传图片，节点内展示「拆分」按钮，
 * 点击弹出对话框，用 fabric.js 画布框选 + 连通域检测拆分成多张切片，
 * 保存后切片上传写入 data.output.images，供下游使用。
 *
...
export default function UiSplitterNode({ id, data, selected }) {
  ...
  const upstreamImages = rawUpstream.slice(0, 1); // 单输入，连线只取首张
  ...
  // FileUpload onChange：单图，上传拿 http URL（同 ImageEditorNode）
  ...
  return (
    <NodeShell id={id} nodeType={NODE_TYPES.uiSplitter} data={data} selected={selected} targetHandle sourceHandle>
      ...
      <UiSplitterDialog
        open={dialogOpen}
        inputImages={[inputImage].filter(Boolean)}
        onSave={handleSave}
