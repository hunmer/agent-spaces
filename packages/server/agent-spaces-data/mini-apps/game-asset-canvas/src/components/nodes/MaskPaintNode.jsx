...
/**
 * 节点对话框数据持久化（见 handoff.md 约束 #15）：
 * 每图蒙版笔触快照保存到 data.paintData，由 useCanvasState 随 canvas.json 持久化；
 * 关闭对话框/刷新/切工作区后重开仍可恢复。输入图变化时按仍存在 URL 逐图恢复。
 */

/**
 * 蒙版绘制节点：接收上游图或本地上传图片，弹出 fabric.js 编辑器，
 * 用画笔（可调大小）/自由套索选区/矩形选区涂抹蒙版（白色=选中区域），
 * 导出后把每张图的黑白蒙版图上传写入 data.output.images，供下游 edit_image 等消费。
 *
 * 输入来源（两种合并去重，多图）：
 * 1. FileUpload 用户上传（data.uploadedImages: string[]，持久化）
 * 2. 上游连线推入（data.images: string[]，由 computeInputImages 派生）
 *
 * 依赖：vendor/fabric.min.js（fabric@5.3.0 IIFE，经 cdn.js 加载）。
 */
export default function MaskPaintNode({ id, data, selected }) {
  ...
  const paintData = data?.paintData || null;
  const [dialogOpen, setDialogOpen] = useState(false);

  // FileUpload onChange：多图，对每个新文件调 uploadFile 拿 http URL
  const handleFilesChange = useCallback(async (files) => {
    ...
    if (pending.length) {
      ...
      try {
        ...
        console.error('MaskPaint upload failed:', err);
        onUpdate?.({ uploading: false, uploadError: err?.message || String(err) });
        return;
      }
    }
    // 输入图变化 → 清旧蒙版快照（持久化规范：坐标套到新图无意义）
    onUpdate?.({ uploadedImages: urls, uploading: false, error: undefined, paintData: null });
  }, [onUpdate]);
  ...
  // 对话框业务数据变化写回节点（持久化到 canvas.json）
  const handlePaintDataChange = useCallback((next) => {
    onUpdate?.({ paintData: next });
  }, [onUpdate]);
  ...
  return (
    <NodeShell id={id} nodeType={NODE_TYPES.maskPaint} data={data} selected={selected} targetHandle sourceHandle>
      {/* 输入图：FileUpload 多图上传 */}
      <UploadSection>
        <FileUpload
      ...
      <MaskPaintDialog
        ...
        initialData={paintData}
        onDataChange={handlePaintDataChange}
        onSave={handleSave}
