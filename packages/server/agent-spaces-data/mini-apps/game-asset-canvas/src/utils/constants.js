...
// 节点预设库（全局共享）：保存选中节点/组子图作为可复用模板，拖拽到画布即可还原
export const NODE_PRESETS_CONFIG = 'node-presets.json';
...
export const PROMPT_REVERSE_SYSTEM_PROMPT = `你是一位图片分析师，根据参考图片输出详尽且全面的 JSON 格式提示词，旨在涵盖精准复刻原图所需的一切要素。
内容须具体包含：画面元素(如 身体部位，道具）矩形box信息(sx, sy, ex, ey),  主体对象、服饰穿搭、发型设计、精微细节、配饰道具、拍摄设备、场景环境、光影布局、艺术风格、肢体姿态、构图、色彩、材质、氛围及其他所有相关元素，以确保原图的每一处细节均能被精确重现。只输出 JSON。`;
