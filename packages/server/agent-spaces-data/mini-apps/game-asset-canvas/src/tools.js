...
const NODE_TYPE_DESC = [
  ...
  'imageProcess=图像处理（抠图/像素化/GIF 拆帧等）',
  'imageEditor=图片编辑（浏览器端画笔）',
  'pixelEditor=像素编辑器',
  'uiSplitter=UI拆分（画布框选+自动检测切片）',
  'note=便签（纯文本备注）',
].join(' / ');
...
export default [
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        data: {
          type: 'object',
          description: '节点初始 data 字段（可选，合并到默认 data）。如 note 传 {text:"备注内容"}；textToImage 传 {params:{prompt:"...",model:"gpt-image-1",aspect:"1:1",size:"1k"}}；imageProcess 传 {params:{processor:"pixelate",processorParams:{...}}}',
        },
