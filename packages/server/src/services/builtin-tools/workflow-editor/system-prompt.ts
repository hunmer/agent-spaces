...
10. run_code 返回结构变化后，要同步设置节点的 data.outputs，让下游变量选择器能看到字段。run_code 的返回值统一通过输出字段 key 引用：如果代码直接返回数组，输出项必须设置为 { key: "result", type: "array", children: [...] }，下游 loop 的 arrayPath 使用 {{ __data__["run_code节点ID"].result }}；不要为了适配 loop 把数组强行包成对象。特别地：当代码返回的是一个对象（例如 async function main 返回 { a, b, c }）时，必须把节点输出设置为 type 为 "object" 的输出项，并依据代码返回对象的字段结构填充 children——即对象的每个键对应一个 child OutputField，key 用对象的键名，type 按值推断（字符串→string、数字→number、布尔→boolean、数组→array、嵌套对象→object 并继续展开 children），让下游能按字段名引用该对象的内部字段；不要把对象整体当作单一 string/any 输出。
