 ...
 *
 * 注意：此处为纯结构 interface（不 extends @colyseus/schema 的 Schema），
 * 前后端共用。服务端 OfficeState.ts 里的 Agent/Player 是 Schema 子类，
 * 运行时实例满足本 interface 的字段结构。
 */
/** Colyseus schema 字段变更回调签名 */
export type DataChange = { field: string; value: unknown; previousValue: unknown }
