...
export interface Workflow {
  ...
  lastRunAt?: number
  /** Epoch ms of the most recent open in editor; undefined if never opened. Used for "last opened" sorting. */
  lastOpenedAt?: number
...
export interface NodeProperty {
  key: string
  label: string
  type: 'text' | 'textarea' | 'number' | 'select' | 'checkbox' | 'code' | 'conditions' | 'array' | 'output_fields' | 'agent' | 'workflow' | 'sqlite' | 'knowledge-base'
  dataType?: DataType
  required?: boolean
  readonly?: boolean
  default?: unknown
