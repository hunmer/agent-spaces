...
export function CreateIssueDialog({ open, onOpenChange, agents = [], defaultTitle, defaultDescription, issue, onSubmit }: CreateIssueDialogProps) {
  ...
  const buildWorkflowPrompt = (workflowDescription: string) => {
    ...
    return [
      ...
      '要求：优先拆分为多个 agent 节点协作；先调用 list_available_agent_capabilities 查询当前环境可用的 mcps/tools/skills 全量清单，再调用 list_agent_capabilities 查询当前可用 agent 及其已配置能力，最后根据每个 agent 的职责分配能力。把 channel/mcp/tool 理解为 agent 的协作与能力配置，不要把它们当成独立节点来创建。例如：为某个 agent_run 节点内的 agent 配置 mcps、tools、skills，并在提示词中要求它通过 channel 同步进度和结果。',
