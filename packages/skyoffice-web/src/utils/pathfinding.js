export function tilesCoveredByRect(x, y, width, height, tileWidth, tileHeight) {
    const tiles = [];
...
export function findGridPath(width, height, start, target, isBlocked) {
    const key = (point) => point.y * width + point.x;
    ...
    if (!previous.has(targetKey))
        return [];
    const path = [];
    for (let current = target; current; current = previous.get(key(current))) {
        path.push(current);
    }
    return path.reverse();
}
